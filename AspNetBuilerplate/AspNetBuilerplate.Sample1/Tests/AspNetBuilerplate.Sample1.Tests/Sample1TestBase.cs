...
namespace AspNetBuilerplate.Sample1.Tests
{
    public abstract class Sample1TestBase : AbpIntegratedTestBase<Sample1TestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected Sample1TestBase()
        {
        ...
        protected void UsingDbContext(Action<Sample1DbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<Sample1DbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<Sample1DbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<Sample1DbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<Sample1DbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Sample1DbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<Sample1DbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Sample1DbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<Sample1DbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<Sample1DbContext, Task<T>> func)
        {
            T result;
